...
public class Queen extends Piece {
    ...
    Queen(Color color) {
        if (color == Color.BLACK) {
            super.icon = new ImageIcon(getClass().getResource("/bQueen.png"));
        } else {
            super.icon = new ImageIcon(getClass().getResource("/wQueen.png"));
        }
    ...
    public boolean CheckValidity(Square from, Square to) {
        ...
        if (Math.abs(x) == Math.abs(y)) {
            return true;
        } else if ((to.x == from.x) && Math.abs(y) > 0) {
            return true;
        } else if ((to.y == from.y) && Math.abs(x) > 0) {
            return true;
        } else {
            return false;
        }
